const elements = {
    void: {
        ...
        density: 0, gravity: 0, slip: 0,
        reactions: [],
    },
    wall: {
        id: 1,
        red: 0.5, green: 0.5, blue: 0.5,
        density: 1, gravity: 0, slip: 0,
        immobile: true,
        reactions: [],
    },
    sand: {
        ...
        density: 0.7, gravity: 0.8, slip: 0.1,
        reactions: [],
    },
    fire: {
        id: 3,
        red: 0.9, green: 0.2, blue: 0.1,
        density: -0.5, gravity: -0.4, slip: 0.3,
        reactions: [],
    },
    torch: {
        id: 3,
    ...
    water: {
        ...
        density: 0.5, gravity: 0.8, slip: 0.8,
        reactions: [],
    },
    plant: {
        id: 4,
    ...
    growingPlantHead: {
        id: 5,
        red: 0.2, green: 0.8, blue: 0, //red: 0.2, green: 1, blue: 0,
        density: 1, gravity: 0, slip: 0,
        immobile: true,
        reactions: [],
        hidden: true,
    ...
    growingPlantTail: {
        id: 6,
        red: 0.2, green: 0.8, blue: 0, //red: 0.2, green: 0.9, blue: 0,
        density: 1, gravity: 0, slip: 0,
        immobile: true,
        reactions: [],
        hidden: true,
...
elements.plant.reactions.push({
    chance: 0.5,
    becomes: elements.growingPlantHead,
    singleNeighbor: {
        element: elements.water,
        becomes: elements.growingPlantHead,
        minimum: 2,
...
elements.growingPlantHead.reactions.push({
    chance: 0.75,
    becomes: elements.growingPlantHead,
    singleNeighbor: {
...
elements.growingPlantHead.reactions.push({
    chance: 0.1,
    becomes: elements.growingPlantTail,
    singleNeighbor: {
        element: elements.water,
        becomes: elements.growingPlantHead,
        minimum: 1,
...
elements.growingPlantTail.reactions.push({
    self: true,
    chance: 0.015,
    becomes: elements.plant,
});
...
function updateBuffers(timestamp, timeBuffer, readBuffer, writeBuffer){
    ...
    while(i >= 0 && i < totalCells){
        ...
        if(!cell) console.log(i);
        if(
            cell.gravity > 0 &&
            south < totalCells &&
            timeBuffer[south] < timestamp &&
            !readBuffer[south].immobile &&
            readBuffer[south].density <= cell.density
        ){
            if(cell.gravity > random){
            ...
            readBuffer[north].density >= cell.density
        ){
            if(cell.gravity < -random){
                ...
                timeBuffer[i] = timestamp;
                timeBuffer[north] = timestamp;
                console.log("It fell up!?");
            }
            random = Math.random();
        }else if(cell.slip > random){
            const westAllowed = (
        ...
        if(timeBuffer[i] < timestamp){
            ...
            if(cell.reactions.length){
                ...
                if(southwest > 0) neighbors[readBuffer[southwest].id] = (
                    (neighbors[readBuffer[southwest].id] || 0) + 1
                );
                if(south > 0) neighbors[readBuffer[south].id] = (
                    (neighbors[readBuffer[south].id] || 0) + 1
                );
                if(southeast > 0) neighbors[readBuffer[southeast].id] = (
                ...
                for(const reaction of cell.reactions){
                    if(reaction.singleNeighbor && reaction.chance > random){
                        ...
                        if(
                            ...
                            timeBuffer[i] = timestamp;
                            const neighborIndexes = neighborOrders[neighborOrderIndex++];
                            if(neighborOrderIndex >= neighborOrders.length){
                    ...
                    }else if(reaction.self && reaction.chance > random){
                        writeBuffer[i] = reaction.becomes;
                        timeBuffer[i] = timestamp;
                    }
                    random = Math.random()
                }
            }else if(cell.gravity){
                random = Math.random();
            }
        }
        i += increment;
